 ...
 * @since      v0.0.1
 */
public class MonthEntryLoader extends AbstractDataSetLoader<MonthEntry> {
	...
	public void load(Map<DataEntryType, DataSetLoader<?>> supportedTypes, LoadedTypeMap loadedType) {
		...
		DataSet<MonthEntry> ds = this.loadFiles(this.getDataEntryType());
		if(ds==null){
	...
	public DataEntryType getDataEntryType() {
		return MonthEntry.ENTRY_TYPE;
	}

	@Override
	public DataSet<MonthEntry> newSetInstance() {
		return new DataSet<>(this.getCs(), this.getEntryFactory());
	}

	@Override
	public DataEntryFactory<MonthEntry> getEntryFactory() {
		return new DataEntryFactory<MonthEntry>() {

			@Override
			public MonthEntry newInstanceLoaded(String keyStart, Map<String, Object> data) throws URISyntaxException {
				MonthEntry ae = this.newInstance();
				ae.load(keyStart, data, getCs());
				return ae;
			}

			@Override
			public MonthEntry newInstance() {
				return new MonthEntry(getLoadedTypes());
			}
		};
	}
}
