...
/**
 * Loader for day-of-week.
 *
 * @author     Sven van der Meer &lt;vdmeer.sven@mykolab.com&gt;
 * @version    v0.0.1 build 151209 (09-Dec-15) for Java 1.8
 * @since      v0.0.1
 */
public class DayofweekEntryLoader extends AbstractDataSetLoader<DayofweekEntry> {
	...
	public void load(Map<DataEntryType, DataSetLoader<?>> supportedTypes, LoadedTypeMap loadedType) {
		...
		DataSet<DayofweekEntry> ds = this.loadFiles(this.getDataEntryType());
		if(ds==null){
	...
	public DataEntryType getDataEntryType() {
		return DayofweekEntry.ENTRY_TYPE;
	}

	@Override
	public DataSet<DayofweekEntry> newSetInstance() {
		return new DataSet<>(this.getCs(), this.getEntryFactory());
	}

	@Override
	public DataEntryFactory<DayofweekEntry> getEntryFactory() {
		return new DataEntryFactory<DayofweekEntry>() {

			@Override
			public DayofweekEntry newInstanceLoaded(String keyStart, Map<String, Object> data) throws URISyntaxException {
				DayofweekEntry ae = this.newInstance();
				ae.load(keyStart, data, getCs());
				return ae;
			}

			@Override
			public DayofweekEntry newInstance() {
				return new DayofweekEntry(getLoadedTypes());
			}
		};
	}
}
