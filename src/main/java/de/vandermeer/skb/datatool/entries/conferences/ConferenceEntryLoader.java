...
/**
 * Loader for conferences.
 *
 * @author     Sven van der Meer &lt;vdmeer.sven@mykolab.com&gt;
 * @version    v0.0.2-SNAPSHOT build 170404 (04-Apr-17) for Java 1.8
 * @since      v0.0.1
 */
public class ConferenceEntryLoader extends AbstractDataSetLoader<ConferenceEntry> {
	...
	public void load(Map<DataEntryType, DataSetLoader<?>> supportedTypes, LoadedTypeMap loadedType) {
		...
		DataSet<ConferenceEntry> ds = this.loadFiles(this.getDataEntryType());
		if(ds==null){
	...
	public DataEntryType getDataEntryType() {
		return ConferenceEntry.ENTRY_TYPE;
	}

	@Override
	public DataSet<ConferenceEntry> newSetInstance() {
		return new DataSet<>(this.getCs(), this.getEntryFactory());
	}

	@Override
	public DataEntryFactory<ConferenceEntry> getEntryFactory() {
		return new DataEntryFactory<ConferenceEntry>() {

			@Override
			public ConferenceEntry newInstanceLoaded(String keyStart, Map<String, Object> data) throws URISyntaxException {
				ConferenceEntry ae = this.newInstance();
				ae.load(keyStart, data, getCs());
				return ae;
			}

			@Override
			public ConferenceEntry newInstance() {
				return new ConferenceEntry(getLoadedTypes());
			}
		};
	}
