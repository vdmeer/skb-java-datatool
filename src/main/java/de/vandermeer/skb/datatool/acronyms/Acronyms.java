...
/**
 * Acronym executable service.
 *
 * @author     Sven van der Meer &lt;vdmeer.sven@mykolab.com&gt;
 * @version    v0.0.6 build 150812 (12-Aug-15) for Java 1.8
 * @since      v0.0.1
 */
public class Acronyms implements ExecS_Application {

	/** Application name. */
	public final static String APP_NAME = "skb-dt-acronyms";

	/** Application display name. */
	public final static String APP_DISPLAY_NAME = "SKB Datatools: Acronyms";

	/** Application version, should be same as the version in the class JavaDoc. */
	public final static String APP_VERSION = "v0.0.1 build 150812 (12-Aug-15) for Java 1.8";

	/** Application description. */
	public final static String APP_DESCR = "Reads acronyms from JSON files, validates them, and creates various output formats.";
	...
	protected AO_DirectoryIn optionDirIn = new AO_DirectoryIn(true, "The top level directory with JSON files for acronyms.");
	...
	protected AO_KeySeparator optioneKeySep = new AO_KeySeparator(':', "The separator for key elements, default is ':'.");

	/** Flag for verbose mode, true means on, false means off. */
	boolean verbose;

	/** The tools' targets. */
	Map<String, AcronymTarget> targets;

	/**
	 * Returns a new Acronym object.
	...
	public Acronyms() {
		...
		this.cli.addOption(this.optioneKeySep);
	...
	public int executeApplication(String[] args) {
		...
		AcronymTarget target = this.targets.get(targetCli);
		if(target==null){
		...
		if(this.verbose){
			Skb_Console.conInfo("{}: processing acronyms for target <{}> writing to <{}>", new Object[]{this.getAppName(), target.getTargetName(), (ft==null)?"standard out":ft.getAbsoluteName()});
		}

		try{
			ret = this.compile(dl, this.optioneKeySep.getValue(), target, ft);
		}
	...
	/**
	 * Compiles all information about found acronyms.
	 * @param dl a directory loader with set filters to read files from
	 * @param keySep key separator
	 * @param target target environment
	 * @param ft output file target
	 * @return 0 on success, -1 on error
	 * @throws FileNotFoundException if the file could not be written to
	 */
	protected int compile(DirectoryLoader dl, char keySep, AcronymTarget target, FileTarget ft) throws FileNotFoundException {
		AcronymSet as = new AcronymSet(this.getAppName(), keySep);
		as.load(dl, target.getTranslationTarget());

		//TODO validate entries, key w/o spaces!
//		for(AcronymEntry ae : acronyms){
//			System.err.println(ae.getShort() + "   -> " + ae.getKey());
//		}

		if(this.verbose){
			Skb_Console.conInfo("{}: parsed <{}> acronyms with <{}> unique keys from <{}> files", new Object[]{this.getAppName(), as.getAcronyms().size(), as.getKeys().size(), as.getFileNumber()});
		}
		...
		st.add("maxAcr", as.getLongestAcronym());
		st.add("maxAcrInt", as.getLongestAcronym().length());
		for(AcronymEntry ae : as.getAcronyms()){
			st.add("ae", ae);
		}
	 ...
	 */
	public void addTarget(AcronymTarget target){
		if(target!=null){
			this.targets.put(target.getTargetName(), target);
			StrBuilder sb = new StrBuilder(100);
			for(AcronymTarget et : this.targets.values()){
				sb.append("   - ").append(et.getTargetName()).append(": ").append(et.getDescription());
			}
			this.optionTarget.setDescriptionLong("The target defines the output target for the acronym tool. Supported targets are:\n" + sb);
